...
-- 예) 2014년 07월 10일
select min(DATE_FORMAT(hire_date,'%Y년 %m월 %d일'))
from employees;
...
select DATE_FORMAT(curdate(),'%Y') - DATE_FORMAT(max(birth_date),'%Y') as 어린사원,
DATE_FORMAT(curdate(),'%Y') - DATE_FORMAT(min(birth_date),'%Y') as 연장자사원
from employees
